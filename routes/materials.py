...
async def read_materials(skip: int=0, limit: int=10, db: Session= Depends(get_db), current_user: schemas.users.User = Depends(get_current_user)):
...
async def read_material(id: int, db:Session=Depends(get_db), current_user: schemas.users.User = Depends(get_current_user)):
...
async def update_material(id: int, material: schemas.materials.MaterialUpdate, db:Session=Depends(get_db), current_user: schemas.users.User = Depends(get_current_user)):
...
async def delete_material(id: int, db:Session=Depends(get_db), current_user: schemas.users.User = Depends(get_current_user)):
